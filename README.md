...
> This repository now contains **two independent Streamlit applications**.
> The existing Trend Intelligence entry point remains `app.py`. The new HULA
> Marketing Operations first-release entry point is
> `apps/marketing_operations.py`.
...
Start Marketing Operations independently on a second port:
...
remain separate from Shopify's 1,459 add-to-carts and 84 online orders. Paid CAC
is shown as unavailable; HK$182.03 is labelled only as paid spend per all new
customers.
