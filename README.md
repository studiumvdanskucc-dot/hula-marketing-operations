...
Current build: **2026.07.22.7**. The build number appears at the bottom of the app sidebar so you can confirm that the Streamlit-ready version is running.
...
If an older copy is already running, stop it with `Control + C` before starting this folder. Seeing **Build 2026.07.22.7** at the bottom of the sidebar confirms that Terminal opened the updated build.
...
Each request explicitly uses `geo=HK`. A full live refresh uses three multi-term comparisons for up to 12 primary terms plus two related-query searches: at most five SerpApi searches. The connection test uses one search. The current free SerpApi plan provides 250 searches per month, which is comfortably above the supplied weekly schedule.
...
Category-only and vague labels are removed before scoring, display, product matching or AI enrichment. The app stores a complete audit for each refresh under **Data & Setup → Trend quality filter**. `bags`, `trousers` and `garments` are removed; `black bags`, `red trousers`, `raffia bags` and `east–west bags` remain valid because they contain a defining attribute.
