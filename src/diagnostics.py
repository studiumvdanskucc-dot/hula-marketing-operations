...
def hybrid_explanation(meta: dict[str, Any]) -> str:
    ...
    if catalogue_source == "csv" or str(statuses.get("shopify", "")).lower().startswith(
        "csv snapshot"
    ):
        expected.append("the catalogue is an uploaded CSV snapshot")

    for key in ("google_trends", "x_apify", "openrouter"):
...
def source_diagnostic_rows(
    ...
    *,
    ...
    for source in ("google_trends", "x_apify", "shopify", "openrouter"):
...
def diagnostic_report(
    ...
    *,
    ...
    return {
        ...
        "privacy": "No API keys, tokens, passwords, client secrets or raw X posts are included.",
    }
